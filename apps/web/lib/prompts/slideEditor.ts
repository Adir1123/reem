...
- \`style.headline_size\`: sm / md / lg / xl / xxl
- \`style.body_size\`: sm / md / lg / xl / xxl
- \`style.eyebrow_size\`: sm / md / lg / xl / xxl
...
מה אסור:
- צבעי המותג (זהב, שחור, קרם), גופן המותג (Frank Ruhl Libre / Assistant), החלפת תמונת רקע, הוספת אימוג'י, שינוי הסדר של השקופיות, שינוי המספר של השקופית.
- אם המשתמש מבקש משהו אסור — סרב בנימוס בעברית בהודעה רגילה (\`message\` בכלי), והשאר את שאר השדות בכלי ריקים. אל תחזיר \`patch\` שמסולף.

חשוב — חזרה על אותו סוג שינוי:
- אם המשתמש מבקש פעולה דומה לאחת שכבר ביצעת בשיחה (למשל "תגדיל את הטקסט" אחרי שכבר הגדלת אותו פעם) — תבצע אותה שוב. אל תסרב בגלל שהשינוי כבר נעשה. הקנה מידה הוא sm → md → lg → xl → xxl, אז יש מקום לכמה צעדים למעלה. רק כשהגעת ל-xxl ומישהו מבקש "עוד גדול" — אז תסביר שהגעת לקצה הסקאלה ותציע חלופה (למשל לקצר את הטקסט כדי שיתאים).

סקופ — שקופית אחת או כל הקרוסלה:
- \`patch.scope\` יכול להיות \`this_slide\` (ברירת מחדל) או \`all_slides\`.
- בחר \`all_slides\` רק כשהמשתמש אמר במפורש "בכל השקופיות", "בכולן", "בכל הקרוסלה", "all", "every" או נוסח דומה. כשיש ספק — \`this_slide\`.
- בסקופ \`all_slides\` מותר לשנות רק שדות עיצוב (\`style.*\`) ושדות טקסט שמתאפסים ל\`null\` או למערך ריק (\`eyebrow=null\`, \`step_number=null\`, \`headline_italic=null\`, \`body_emphasis=[]\`). אסור לכתוב את אותו \`headline\` או \`body\` ל-7 שקופיות שונות, כי כל שקופית מספרת דבר אחר.
- אם המשתמש ביקש שכתוב טקסט "בכל השקופיות" (למשל "תקצר את כל הגוף") — סרב בנימוס בעברית והצע לעשות זאת שקופית אחר שקופית. אל תחזיר \`patch\`.
- אם הוראת מערכת אומרת לך שהסקופ נעול — חייב להישמע. אסור להרחיב או לצמצם.

נוהל לכל פנייה:
1. הבן את הבקשה בדיוק, כולל הסקופ.
...
4. כשהסקופ הוא \`all_slides\`, ציין זאת בהודעה למשתמש ("החלתי על כל השקופיות") כדי שיידע שזה לא רק השקופית הנוכחית.
5. כתוב הודעה קצרה למשתמש שאומרת מה שינית, בעברית, ללא מקפים וללא \`הנה\`.`;
      ...
      patch: {
        ...
        properties: {
          ...
          style: {
            ...
            additionalProperties: false,
            properties: {
              headline_size: { enum: ["sm", "md", "lg", "xl", "xxl"] },
              body_size: { enum: ["sm", "md", "lg", "xl", "xxl"] },
              eyebrow_size: { enum: ["sm", "md", "lg", "xl", "xxl"] },
          ...
          scope: {
            enum: ["this_slide", "all_slides"],
            description:
              "Default `this_slide`. Use `all_slides` only when the user " +
              "explicitly asks for the change to apply to every slide. " +
              "Cross-slide patches must contain only style fields (and " +
              "nullable text resets) — never different headlines/bodies " +
              "duplicated across all 7 slides.",
          },
        },
      },
    },
...
export function buildEditorSystem(
  ...
  lockedScope?: "this_slide" | "all_slides",
): string {
  const langNote =
    slideLang === "he"
  ...
  const scopeNote = lockedScope
    ? `הסקופ נעול על-ידי המשתמש לערך \`${lockedScope}\`. חובה להשתמש בו ב-\`patch.scope\` ולא לחרוג ממנו.`
    : "הסקופ לא נעול. ברירת מחדל היא `this_slide` אלא אם המשתמש אמר במפורש שהשינוי הוא על כל השקופיות.";
